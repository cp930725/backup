...
use app\api\controller\Configure;
...
class Event extends Base
{
    ...
    public function shipped(Request $req)
    {
        ...
        $bool = Db::table('event_log')->where('id', '=', $id)->update([
            'status'        =>  \app\api\controller\Event::STATUS_SHIPPED,
            'send'          =>  $send,
    ...
    public function contract_store(Request $req)
    {
        ...
        if ($req->isPost()) {
            ...
            $action = $req->param('action');
            // 合约对象
            $ct = new \app\api\controller\Contract();
            // 获取数据
            if ($action == 'get') {
                $id = $req->param('id/d');
                if (empty($id)) {
    ...
    public function ticket(Request $req)
    {
        ...
        if ($action == 'create') {
            try {
                ...
                // 创建数据
                $count = (new \app\api\controller\Ticket())->create($type, $number);
            } catch (\Exception $e) {
                $this->error($e->getMessage());
                exit;
            }
            ...
            try {
                // 删除数据
                (new \app\api\controller\Ticket())->delete($req->param('token'));
            } catch (\Exception $e) {
                $this->error($e->getMessage());
                exit;
            }
