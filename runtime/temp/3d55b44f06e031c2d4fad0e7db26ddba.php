<?php /*a:2:{s:68:"/www/wwwroot/aa.jdswzc.com/application/admin/view/event/funding.html";i:1562578227;s:67:"/www/wwwroot/aa.jdswzc.com/application/admin/view/common/world.html";i:1562574822;}*/ ?>
<!doctype html>
<html lang="en">
<head>
    <meta charset="utf-8" />
            ...
            <tbody>
                <?php if(is_array($logs) || $logs instanceof \think\Collection || $logs instanceof \think\Paginator): $i = 0; $__LIST__ = $logs;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$log): $mod = ($i % 2 );++$i;?>
                    <tr>
                        ...
                        <td><span class="badge badge-default badge-md"><?php echo htmlentities(app('config')->get('hello.funding.catalog')[$log['catalog']]); ?></span></td>
                        <td>
...
require(['jquery', 'ZeroClipboard'], function($, ZeroClipboard){
    ...
    $(function(){
        ...
        $('.btn-edit').on('click', function(){
            ...
            var id = $(this).data('id');
            $('.modal-update input[name="min[]"]').val('');
            $('.modal-update input[name="max[]"]').val('');
            $('.modal-update input[name="bonus[]"]').val('');
            ajax('/admin/event/funding', {action: 'get', id: id}, function(res){
                ...
                if (res.data.bonus) {
                    if ((res.data.bonus.split(';').length-1) > 1) {
                        ...
                        $.each(arr, function (key, value) {
                            ...
                            var val = value.split(',');
                            $('.modal-update input[name="min[]"]').eq(key).val(val[0]);
                        ...
                        var arr = bonus.split(',');
                        $('.modal-update input[name="min[]"]').val(arr[0]);
                        $('.modal-update input[name="max[]"]').val(arr[1]);
                        $('.modal-update input[name="bonus[]"]').val(arr[2]);
                    }
                }
                if (res.data.visible) {
