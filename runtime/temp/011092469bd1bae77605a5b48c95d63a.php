<?php /*a:2:{s:72:"/www/wwwroot/aa.jdswzc.com/application/admin/view/event/funding_log.html";i:1562638176;s:67:"/www/wwwroot/aa.jdswzc.com/application/admin/view/common/world.html";i:1562662267;}*/ ?>
<!doctype html>
<html lang="en">
<head>
    <meta charset="utf-8" />
        ...
        <div class="col-md-6 col-lg-3 mb-3">
            <div class="input-group">
                ...
                <select class="custom-select input-group-text" name="fid">
                    ...
                    <?php if(is_array($fids) || $fids instanceof \think\Collection || $fids instanceof \think\Paginator): $i = 0; $__LIST__ = $fids;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;if(is_null(app('request')->get('fid')) || app('request')->get('fid') == ''): ?>
                            <option value="<?php echo htmlentities($item['id']); ?>"><?php echo htmlentities($item['title']); ?></option>
                        <?php else: if(app('request')->get('fid') == $item['id']): ?>
        ...
        <table class="table table-hover table-outline table-vcenter card-table">
            ...
            <tbody>
                <?php if(is_array($logs) || $logs instanceof \think\Collection || $logs instanceof \think\Paginator): $i = 0; $__LIST__ = $logs;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$log): $mod = ($i % 2 );++$i;?>
                    <tr {eq name="log.status" value="1"}style="success"{\eq} {eq name="log.status" value="3"}style="danger"{\eq}>
                        <td><?php echo htmlentities($log['title']); ?></td>
                        <td><?php echo htmlentities($log['username']); ?></td>
                        <td>
                            <span class="tag">
                            <?php switch($log['action']): case "1": ?>投资<?php break; case "8": ?>结束<?php break; default: ?> 其他
                            <?php endswitch; ?>
                            </span>
                        </td>
                        ...
                        <td>
                            <?php if($log['status'] == '2'): ?>
                            <a href="/admin/event/funding_ok.html?id=<?php echo htmlentities($log['id']); ?>" class="btn btn-green btn-sm">通过</a>
                            <a href="/admin/event/funding_no.html?id=<?php echo htmlentities($log['id']); ?>" class="btn btn-danger btn-sm ml-2">拒绝</a>
                            <?php endif; ?>
                        </td>
                    </tr>
