<?php /*a:2:{s:73:"/www/wwwroot/aa.jdswzc.com/application/admin/view/store/adjust_price.html";i:1562574822;s:67:"/www/wwwroot/aa.jdswzc.com/application/admin/view/common/world.html";i:1562662267;}*/ ?>
<!doctype html>
<html lang="en">
<head>
    <meta charset="utf-8" />
...
<form method="post">
    <div class="row">
        <div class="col-lg-3 mb-3">
            <select name="catalog" class="form-control custom-select">
                ...
                <?php if(is_array(app('config')->get('hello.store.catalog')) || app('config')->get('hello.store.catalog') instanceof \think\Collection || app('config')->get('hello.store.catalog') instanceof \think\Paginator): $catalogId = 0; $__LIST__ = app('config')->get('hello.store.catalog');if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$catalogName): $mod = ($catalogId % 2 );++$catalogId;?>
                    <option value="<?php echo htmlentities($catalogId); ?>">针对<?php echo htmlentities($catalogName); ?></option>
    ...
    <div class="row mt-3">
        <?php if(is_array(app('config')->get('hello.store.catalog')) || app('config')->get('hello.store.catalog') instanceof \think\Collection || app('config')->get('hello.store.catalog') instanceof \think\Paginator): $catalogId = 0; $__LIST__ = app('config')->get('hello.store.catalog');if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$catalogName): $mod = ($catalogId % 2 );++$catalogId;?>
            <div class="col-lg-4 catalog-<?php echo htmlentities($catalogId); ?>">
                <div class="card">
                    <div class="card-header p-3">
                        <div class="font-weight-light text-danger"><?php echo htmlentities($catalogName); ?></div>
                    </div>
                    <div class="list p-3">
                        <?php if(is_array($store) || $store instanceof \think\Collection || $store instanceof \think\Paginator): $i = 0; $__LIST__ = $store;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;if($item['catalog'] == $catalogId): ?>
                                <div class="form-group">
                                    <input type="hidden" name="id[]" value="<?php echo htmlentities($item['id']); ?>" />
                                    <label class="from-label"><?php echo htmlentities($item['title']); ?></label>
                                    <input type="text" name="price[]" class="form-control price" value="<?php echo htmlentities(money($item['price'])); ?>" />
                                </div>
